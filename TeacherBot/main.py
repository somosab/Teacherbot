...
# Flask health-check server for self-ping and Replit uptime
shutdown_event = threading.Event()
PING_INTERVAL_SECONDS = 300
FLASK_PORT = int(os.environ.get("PORT", "5000"))
PING_URL = f"http://localhost:{FLASK_PORT}/"
...
async def ping_self_async() -> None:
    """Background task that pings the local Flask endpoint every 5 minutes."""
...
def ping_self() -> None:
    asyncio.run(ping_self_async())


def run_flask_server() -> None:
    try:
        logger.info("Starting Flask health server on port %s", FLASK_PORT)
        web_app.run(host="0.0.0.0", port=FLASK_PORT, threaded=True, use_reloader=False)
    except Exception:
        logger.exception("Flask web server failed to start")
...
async def send_lesson(update, context, understood=True):
    ...
    # Enforce: each next lesson (kun > 1) has at least 2 posts
    if kun > 1 and total_posts < 2:
        total_posts = 2
        context.user_data["lesson_total_posts"] = total_posts

    # If this is the first post of the day and we have a previous lesson,
    # instruct the model to produce a short recap of the previous lesson.
    # Preserve links by including the previous lesson text without truncation.
    if current_post == 1 and previous:
        extra = (
            "\nFirst post: Please write a short recap/revision of the previous lesson. "
            "Keep any links and examples intact.\nPrevious lesson:\n" + previous + "\n"
        )
    else:
        extra = (
    ...
    template += (
        "\n\n📝 EXPLANATION:\n"
        "Add a short clear explanation of the lesson topic.\n\n"
        "🧩 ADDITIONAL EXERCISES:\n"
        "Include 3 additional practice exercises related to this lesson."
    )
    if total_posts > 1:
    ...
    try:
            ...
            if text == "Ha, chiqaman":
                ...
                if course_id:
                    try:
                        ...
                        logger.exception("Failed to delete course %s for user %s", course_id, update.effective_user.id)
                ...
                context.user_data.pop("awaiting_exit_confirmation", None)
                context.user_data.pop("exit_course_id", None)
                await update.message.reply_text("Kurs o'chirildi va bosh menyuga qaytildi. /start ni bosing")
        ...
        if text == "⚙️ SETTINGS":
            settings_keyboard = ReplyKeyboardMarkup(
                [["🔄 Kursni davom ettirish"], ["🗑️ Kursni o'chirish"], ["🏠 Asosiy menyu"]],
                resize_keyboard=True,
            )
            await update.message.reply_text(
                "Sozlamalar: darsni davom ettirish, kursni o'chirish yoki bosh menyuga qaytish.",
                reply_markup=settings_keyboard,
            )
            return

        if text == "🔄 Kursni davom ettirish":
            course_id = get_current_course_id(update, context)
            if not course_id:
                await update.message.reply_text("Kurs topilmadi. /start ni bosing.")
                return
        ...
        if text == "🗑️ Kursni o'chirish":
            ...
            await update.message.reply_text(
                "Kursni o'chirishni xohlaysizmi? Bu amal qaytarib bo'lmaydi.",
                reply_markup=keyboard,
            )
            return
        ...
        if text in LANGUAGES:
            ...
            context.user_data["current_post"] = 1
            context.user_data.pop("next_day_unlock", None)
            # Ensure next lessons have at least 2 posts
            context.user_data["lesson_total_posts"] = max(int(context.user_data.get("lesson_total_posts", 1)), 2)
            await send_lesson(update, context, understood=True)
...
def main() -> None:
    flask_thread = threading.Thread(target=run_flask_server, name="FlaskThread", daemon=True)
    ping_thread = threading.Thread(target=ping_self, name="PingThread", daemon=True)
